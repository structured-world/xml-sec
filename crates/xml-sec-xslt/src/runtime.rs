...
struct PreparedParameters<'a> {
    effective_globals: HashMap<ExpandedName, &'a crate::compiler::GlobalVariable>,
    source_remap: SourceParameterRemap,
}

struct AttributeSetExpansion<'a> {
    declarations: &'a [AttributeSet],
    active: Vec<&'a ExpandedName>,
    reserved_owned_bytes: usize,
}

impl<'a> AttributeSetExpansion<'a> {
    const fn new(declarations: &'a [AttributeSet]) -> Self {
        Self {
            declarations,
            active: Vec::new(),
    ...
    fn push(&mut self, name: &'a ExpandedName, meter: &mut Meter) -> Result<()> {
        if self.active.contains(&name) {
            return Err(Error::Static("attribute-set cycle".into()));
        }
        meter.recursion_with_ceiling(
            self.active.len().saturating_add(1),
            EXECUTION_RECURSION_DEPTH_CEILING,
        )?;
        reserve_temporary_vec_slot(&mut self.active, meter, &mut self.reserved_owned_bytes)?;
...
impl Stylesheet {
    ...
    pub fn execute_with_environment_and_source_processing<R: Resolver + 'static>(
        ...
        for (name, value) in parameters {
            if effective_globals
                .get(name)
                .is_some_and(|global| global.is_parameter)
            {
        ...
        let prepared_parameters = PreparedParameters {
            effective_globals,
            source_remap: SourceParameterRemap {
...
impl<'a> Execution<'a> {
    fn new<R: Resolver + 'static>(
        ...
        parameters: &Parameters,
        prepared_parameters: PreparedParameters<'a>,
        environment: ExecutionEnvironment<R>,
        mut meter: Meter,
        ...
        let evaluator = Evaluator::new(
            ...
            stylesheet.principal_base_uri.as_deref(),
        ...
        let PreparedParameters {
            effective_globals,
            source_remap:
                SourceParameterRemap {
                    mapping,
                    owned_bytes,
                },
        } = prepared_parameters;
        let initialized = state.initialize_globals(parameters, mapping.as_ref(), effective_globals);
        drop(mapping);
    ...
    fn apply_attribute_sets(
        ...
        let declarations = Arc::clone(&self.stylesheet.attribute_sets);
        let mut expansion = AttributeSetExpansion::new(&declarations);
        let result = (|| {
            for name in names {
                self.apply_attribute_set(name, node, frame, current_precedence, &mut expansion)?;
            }
            Ok(())
        })();
        self.meter
            .release_owned_bytes(expansion.reserved_owned_bytes);
        result
    }

    fn apply_attribute_set<'s>(
        &mut self,
        name: &'s ExpandedName,
        node: &SourceNode,
        frame: ApplyFrame,
...
fn metered_node_id_snapshot(nodes: &[NodeId], meter: &mut Meter) -> Result<(Vec<NodeId>, usize)> {
    ...
    for node in nodes.iter().copied() {
        if let Err(error) =
            reserve_temporary_vec_slot(&mut snapshot, meter, &mut reserved_owned_bytes)
        {
    ...
    Ok((snapshot, reserved_owned_bytes))
}
...
mod tests {
    ...
    #[test]
    fn logical_root_snapshot_reserves_before_allocation_and_releases_on_failure() {
        // key() snapshots every logical document before mutating key indexes. The temporary node
        // vector must cross OwnedBytes before its first allocation and unwind a partial reserve.
        let nodes = (0..5).map(crate::NodeId::test).collect::<Vec<_>>();
        let first_allocation = 4 * std::mem::size_of::<crate::NodeId>();
        let mut rejected = meter(first_allocation - 1);

        assert!(matches!(
            metered_node_id_snapshot(&nodes, &mut rejected),
            Err(Error::Budget {
                kind: BudgetKind::OwnedBytes,
                ..
            })
    ...
    #[test]
    fn attribute_set_recursion_stack_borrows_names_and_meters_only_pointer_storage() {
        // Attribute-set names belong to the immutable stylesheet. Recursion tracking borrows them
        // and reserves its pointer stack before growth instead of cloning attacker-sized strings.
        let names = (0..5)
            .map(|index| ExpandedName::new(None::<String>, format!("{index}{}", "x".repeat(4096))))
            .collect::<Vec<_>>();
        let first_allocation = 4 * std::mem::size_of::<&ExpandedName>();
        let mut meter = meter(first_allocation);
        let mut expansion = AttributeSetExpansion::new(&[]);
        for name in &names[..4] {
            expansion
                .push(name, &mut meter)
                .expect("four borrowed names fit the first stack allocation");
        }
        assert_eq!(expansion.reserved_owned_bytes, first_allocation);
        assert!(matches!(
            expansion.push(&names[4], &mut meter),
            Err(Error::Budget {
                kind: BudgetKind::OwnedBytes,
                ..
            })
        ));
        meter.release_owned_bytes(expansion.reserved_owned_bytes);
        assert_eq!(
            meter
                .usage(BudgetKind::OwnedBytes)
                .expect("usage is available")
