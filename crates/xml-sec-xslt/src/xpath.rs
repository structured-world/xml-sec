...
impl Evaluator {
    ...
    pub(crate) fn new<R: Resolver + 'static>(
        ...
        let mut node_base_uris = HashMap::new();
        let mut node_base_uri_index_bytes = 0;
        extend_node_base_uri_entries(
            &source,
            maps.reverse.iter(),
            &mut node_base_uris,
            &mut node_base_uri_index_bytes,
            meter,
        )?;
    ...
    fn import_result_tree_fragment(
        ...
        if let Some((root, _owner)) = self.result_tree_fragments.get(&identity) {
            return Ok(root.clone());
        }
        let root = self.import_document(fragment, meter)?;
        reserve_retained_hash_map_slot(
            &mut self.result_tree_fragments,
            meter,
            &mut self.result_tree_fragment_index_bytes,
        )?;
        // The adapter handle is the Arc allocation address. Keep that allocation alive for as
        // long as its imported projection is cached so the allocator cannot recycle the handle
        // for a different fragment during this execution.
        self.result_tree_fragments
            .insert(identity, (root.clone(), Arc::clone(fragment)));
        Ok(root)
    }
    ...
    fn import_document(&mut self, document: &Document, meter: &mut Meter) -> Result<SourceNode> {
        ...
        extend_node_base_uri_entries(
            ...
            &mut self.node_base_uris.borrow_mut(),
            &mut self.node_base_uri_index_bytes,
            meter,
        )?;
    ...
    fn evaluate_core(
        ...
        context.set_function(
            ...
            FormatNumberFunction {
                formats: Arc::clone(&self.decimal_formats),
                namespaces: Arc::clone(&function_namespaces),
            },
        );
        context.set_function(
...
fn classify_xinclude_decode_error(
    error: xml_sec_xml_input::Error,
    mode: XIncludeParseMode,
) -> XIncludeFailure {
    // XInclude 1.0 sections 4.2 and 4.3 both classify a resource in an unsupported encoding as a
    // resource error. Invalid byte sequences and non-well-formed XML remain fatal; the mode is
    // explicit here so those two contracts cannot be accidentally collapsed into parser syntax.
    // https://www.w3.org/TR/xinclude/#xml
    // https://www.w3.org/TR/xinclude/#text_included
    let unsupported_encoding = matches!(
        error,
...
fn extend_node_base_uri_entries<'a>(
    ...
    index: &mut HashMap<Vec<usize>, Option<String>>,
    ...
    for (path, node) in entries {
        ...
        let semantic_node = source
            .node(*id)
            .ok_or_else(|| Error::Dynamic(format!("stale semantic node {id:?}")))?;
        let base_uri = semantic_node.base_uri.as_deref();
        let inherited = semantic_node
            .parent
            .and_then(|parent| source.node(parent))
            .and_then(|parent| parent.base_uri.as_deref());
        let is_logical_root = source.logical_roots().binary_search(id).is_ok();
        if !is_logical_root && base_uri == inherited {
            continue;
        }
        let key = path.ordinary();
        meter.charge(
            BudgetKind::OwnedBytes,
            key.len()
                .saturating_mul(std::mem::size_of::<usize>())
        ...
        reserve_retained_hash_map_slot(index, meter, index_bytes)?;
        index.insert(key.to_vec(), base_uri.map(str::to_owned));
    }
    Ok(())
}

fn node_base_uri_at<'a>(
    index: &'a HashMap<Vec<usize>, Option<String>>,
    path: &NodePath,
) -> Option<&'a str> {
    let mut ordinary = path.ordinary();
    loop {
        if let Some(base_uri) = index.get(ordinary) {
            return base_uri.as_deref();
        }
        if ordinary.len() <= 2 {
            return None;
        }
        ordinary = &ordinary[..ordinary.len() - 1];
    }
}
...
fn extend_id_index(
    ...
    while index.len() < source.logical_roots().len() {
        reserve_retained_vec_slot(index, meter)?;
        ...
        let base_selection = if let Some(base) = args.get(1) {
            ...
            DocumentBaseSelection::Explicit {
                base_uri: clone_metered_optional_string(
                    context,
                    node_base_uri_at(&base_uris, &path),
                )?,
...
mod tests {
    ...
    #[test]
    fn live_result_tree_fragment_handles_do_not_alias() {
        let first = Arc::new(Document::empty(None));
        let first_identity = result_tree_fragment_handle(&first);
        let second = Arc::new(Document::empty(None));
        assert_ne!(first_identity, result_tree_fragment_handle(&second));
    }
    ...
    #[test]
    fn auxiliary_document_indexes_charge_empty_document_slots() {
        ...
        extend_id_index(&source, &maps, 0, &mut ids, &mut meter).expect("empty ID index builds");
        let mut entities = Vec::new();
        append_unparsed_entity_document(&source, source.root(), &mut entities, &mut meter)
            .expect("empty unparsed-entity index builds");
        let charged = meter.usage(BudgetKind::OwnedBytes).expect("valid kind").0 - before;
        let retained_outer_capacity = ids
            .capacity()
            .saturating_mul(std::mem::size_of::<HashMap<String, NodePath>>())
            .saturating_add(
                entities
                    .capacity()
                    .saturating_mul(std::mem::size_of::<HashMap<String, String>>()),
    ...
    #[test]
    fn node_base_uri_index_charges_hash_backing_storage() {
        let source =
            Document::parse("<root xml:base='memory:base'/>", None).expect("source parses");
        let mut meter = Meter::new(
            ExecutionBudget {
                source_bytes: source.source_bytes(),
                external_documents: 0,
        ...
        let path = NodePath::Ordinary(vec![0, 0, 0]);
        let node = SourceNode::Node(element);
        ...
        extend_node_base_uri_entries(
            &source,
            std::iter::once((&path, &node)),
            &mut index,
            &mut index_bytes,
            &mut meter,
        )
        .expect("base URI index builds");

        let charged = meter.usage(BudgetKind::OwnedBytes).expect("valid kind").0 - before;
        assert_eq!(
            index_bytes,
            retained_hash_storage::<(Vec<usize>, Option<String>)>(index.capacity())
        );
        assert!(charged >= index_bytes.saturating_add("memory:base".len()));
    }
    ...
    #[test]
    fn xinclude_decode_errors_preserve_resource_and_fatal_boundaries() {
        // XInclude 1.0 sections 4.2 and 4.3 permit fallback for an unsupported encoding in both
        // parse modes, but malformed bytes remain fatal after an encoding has been selected.
        // https://www.w3.org/TR/xinclude/#xml
        // https://www.w3.org/TR/xinclude/#text_included
        for mode in [XIncludeParseMode::Xml, XIncludeParseMode::Text] {
            assert!(matches!(
                classify_xinclude_decode_error(
                    xml_sec_xml_input::Error::UnsupportedEncoding("unknown".into()),
                    mode,
                ),
                XIncludeFailure::Resource(_)
            ));
            assert!(matches!(
                classify_xinclude_decode_error(
                    xml_sec_xml_input::Error::InvalidBytes("UTF-8"),
                    mode,
                ),
    ...
    #[test]
    fn lang_reserves_nodeset_string_materialization() {
        // A node-set argument can expose an attacker-sized string-value even though lang()
        // returns only a boolean. The conversion must cross the evaluator allocation gate.
        let package = Package::new();
        let document = package.as_document();
        let root = document.create_element("root");
        root.set_attribute_value(("http://www.w3.org/XML/1998/namespace", "lang"), "en-GB");
        root.append_child(document.create_text(&"x".repeat(2_048)));
        document.root().append_child(root);
        ...
        let evaluation = sxd_xpath_no_unsafe::context::Evaluation::new(&xpath_context, item.into());

        let error = LangFunction
            .evaluate(&evaluation, vec![SxdValue::Nodeset(nodes)])
            .expect_err("node string-value exceeds the evaluator allocation budget");
        assert!(matches!(error, function::Error::Other { what } if what.contains("budget")));
    }

    #[test]
    fn lang_matches_ascii_case_and_sublanguage_without_folding() {
        assert!(language_tag_matches("EN-gb", "en"));
