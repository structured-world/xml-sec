...
#[test]
fn explicit_exslt_duration_obeys_the_extension_budget() {
    // Explicit date:duration still invokes an EXSLT extension and must consume extension work
    // even though it does not consult the operation clock.
    let stylesheet = compile(
        r#"<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:date="http://exslt.org/dates-and-times"><xsl:output method="text"/><xsl:template match="/"><xsl:value-of select="date:duration(1)"/></xsl:template></xsl:stylesheet>"#,
    );
    let source = Document::parse("<source/>", None).expect("source parses");
    let mut budget = execution_budget(1024);
    budget.extension_operations = 0;
...
fn exslt_durations_allow_fractional_syntax_only_for_seconds() {
    // XML Schema 1.0 erratum E2-23 requires digits after a decimal point. Duration arithmetic is
    // schema-strict and accepts a fractional spelling only for seconds when that fraction contains
    // at least one digit; the libxslt date:seconds() compatibility quirk remains local to that
    // conversion.
