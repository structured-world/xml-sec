...
impl<'d> Element<'d> {
    ...
    pub fn attributes_len(&self) -> usize {
        self.document.connections.attributes_len(self.node)
    }
...
mod test {
    ...
    #[test]
    fn child_counts_track_root_and_element_children() {
        // Budget preflight needs an exact count without constructing child-handle vectors.
        let package = Package::new();
        let doc = package.as_document();
        let parent = doc.create_element("parent");
        doc.root().append_child(parent);
        parent.append_child(doc.create_element("first"));
        parent.append_child(doc.create_text("second"));

        assert_eq!(doc.root().children_len(), 1);
    ...
    fn removing_nonchild_preserves_parent_and_cycle_checks() {
        // Removal through an unrelated handle must leave both topology edges intact.
        let package = Package::new();
        let doc = package.as_document();
        let parent = doc.create_element("parent");
        let child = doc.create_element("child");
        parent.append_child(child);
        doc.create_element("unrelated").remove_child(child);
        assert_eq!(child.parent().and_then(|node| node.element()), Some(parent));
